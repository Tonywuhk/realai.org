...
$$
  Q^\pi (s,a) \equiv \mathbb{E}[r_1+r_2+... \| S_0=s, A_0=a, \pi].
$$

The optimal value \\(Q^*(s,a) = \max\_\pi Q^\pi(s,a)\\) is achieved when the agent follows an optimal policy \\)\pi^*\\).

To reduce the overestimation problem of the popular Q-learning algorithm, [van Hasselt et al. (2015)](https://arxiv.org/abs/1509.06461) proposed the double DQN algorithm that learns the target

$$
  Y_t^{DQ} \equiv R_{t+1} + \gamma Q(S_{t+1}, \argmax{a} Q(S_{t+1}, a; \theta_t), \theta_t^-).
$$
